...
public final class AosLiveSmokeTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            recorder.check(
                "async.root.info",
                () -> asyncClient.info().get(30, TimeUnit.SECONDS).version().number(),
                notEmpty()
            );
            recorder.check(
            ...
            if (asyncIndexCreated) {
                recorder.check(
                    ...
                    () -> asyncClient.indices()
                        .delete(request -> request.index(asyncIndex))
                        .get(30, TimeUnit.SECONDS)
                        .acknowledged(),
                    Boolean.TRUE::equals
                );
    ...
    private static void runIndexMetadataChecks(OpenSearchClient client, Recorder recorder, String index) {
        ...
        recorder.check(
            "indices.get_mapping",
            () -> client.indices().getMapping(request -> request.index(index)),
            value -> value != null
        );
        recorder.check(
            "indices.get_settings",
            () -> client.indices().getSettings(request -> request.index(index)),
            value -> value != null
        );
        recorder.check(
            "indices.analyze",
            () -> client.indices()
                .analyze(request -> request.index(index).analyzer("standard").text("AOS smoke test"))
                .tokens()
                .size(),
            count -> count > 0
        );
    ...
    private static boolean runDocumentChecks(OpenSearchClient client, Recorder recorder, String index) {
        ...
        if (firstId != null) {
            recorder.check(
                "root.exists",
                () -> client.exists(request -> request.index(index).id(firstId)).value(),
                Boolean.TRUE::equals
            );
            recorder.check(
                "root.get",
        ...
        recorder.check(
            "root.search_shards",
            () -> client.searchShards(request -> request.index(index)),
            value -> value != null
        );
        recorder.check(
            "indices.refresh",
        ...
        recorder.check(
            "indices.flush",
            () -> client.indices().flush(request -> request.index(index)),
            value -> value != null
        );
        recorder.check(
            "cat.indices",
            () -> client.cat().indices(request -> request.index(index)).valueBody().size(),
            count -> count > 0
        );
        recorder.check(
            "cat.count",
            () -> client.cat().count(request -> request.index(index)).valueBody().size(),
            count -> count > 0
        );
        recorder.check(
            "cat.shards",
            () -> client.cat().shards(request -> request.index(index)).valueBody().size(),
            count -> count > 0
        );
        return appendOnly;
    }
    ...
    private static void runAliasChecks(OpenSearchClient client, Recorder recorder, String index, String alias) {
        ...
        recorder.check(
            "cat.aliases",
            () -> client.cat().aliases(request -> request.name(alias)).valueBody().size(),
            count -> count > 0
        );
    }
    ...
    private static void runAsyncDocumentChecks(OpenSearchAsyncClient client, Recorder recorder, String index, boolean appendOnly) {
        ...
        if (documentId != null) {
            ...
            recorder.check(
                "async.root.get",
                () -> client.get(request -> request.index(index).id(documentId), Map.class)
                    .get(30, TimeUnit.SECONDS)
                    .found(),
                Boolean.TRUE::equals
            );
